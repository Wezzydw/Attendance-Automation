...
//        Lise.addAttendanceDate(LocalDate.of(2019, 2, 28));
        ...
        while (b.isBefore(a))
        {
            if (!(b.getDayOfWeek().toString().equals("SATURDAY") || b.getDayOfWeek().toString().equals("SUNDAY")))
            {
                if (Math.random() > 0.98)
                {
                    Tonny.addAttendanceDate(b, "Present");
                } else
                {
                    Tonny.addAttendanceDate(b, "Absent");
                }

                if (Math.random() > 0.05)
                {
                    Anne.addAttendanceDate(b, "Present");
                } else
                {
                    Anne.addAttendanceDate(b, "Absent");
                }

                if (Math.random() > 0.50)
                {
                    Mads.addAttendanceDate(b, "Present");
                } else
                {
                    Mads.addAttendanceDate(b, "Absent");
                }

                if (Math.random() > 0.25)
                {
                    Peter.addAttendanceDate(b, "Present");
                } else
                {
                    Peter.addAttendanceDate(b, "Absent");
                }

                if (Math.random() > 0.75)
                {
                    Trine.addAttendanceDate(b, "Present");
                } else
                {
                    Trine.addAttendanceDate(b, "Absent");
                }

                if (Math.random() > 0.90)
                {
                    Jack.addAttendanceDate(b, "Present");
                } else
                {
                    Jack.addAttendanceDate(b, "Absent");
                }

                if (Math.random() > 0.38)
                {
                    Lise.addAttendanceDate(b, "Present");
                } else
                {
                    Lise.addAttendanceDate(b, "Absent");
                }
            }
